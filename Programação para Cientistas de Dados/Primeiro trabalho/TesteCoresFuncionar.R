...
quickhull = function(df){
  ...
  above = insideTriangle(p1,p2,max_point_above,above)
  below = insideTriangle(p1,p2,max_point_below,below)
  
  convex_hull = quickhull2(convex_hull,above)
  convex_hull = rbind(convex_hull,quickhull2(convex_hull,below))
  
  #convex_hull = final_check(convex_hull)
...
create_lines = function(convex_hull,p1,p2,above,below,max_point_above,max_point_below,sorted){
  #plot sorted points
  plot(sorted, type = "p", col = "blue", pch = 16, cex = 1.5)
  ...
  points(convex_hull, col = "black", pch = 16, cex = 1.5)
...
insideTriangle = function(p1,p2,p3,segment){
  ...
  inside <- sp::point.in.polygon(segment$x, segment$y, triangle[,1], triangle[,2])
...
quickhull2 = function(convex_hull,sorted){
  for(num in 1:nrow(convex_hull)){
    for(j in 1:nrow(convex_hull)){
      ...
      p3 = find_distance(p1,p2,sorted)
      sorted = insideTriangle(p1,p2,p3,sorted)
      convex_hull = rbind(convex_hull,p3) 
...
final_check <- function(convex_hull) {
  
  # Loop through all possible combinations of 3 points in the convex hull
  for(pi1 in 1:nrow(convex_hull)){
    for(pi2 in 1:nrow(convex_hull)){
      for(pi3 in 1:nrow(convex_hull)){
        # Check if the 3 points are distinct (i.e. not the same point)
        p1 = data.frame(x = c(convex_hull[pi1,]$x), y = c(convex_hull[pi1,]$y))
        p2 = data.frame(x = c(convex_hull[pi2,]$x), y = c(convex_hull[pi2,]$y))
        p3 = data.frame(x = c(convex_hull[pi3,]$x), y = c(convex_hull[pi3,]$y))
        if(p1$x != p2$x && p2$x != p3$x && p1$x != p3$x && p1$y != p2$y && p2$y != p3$y && p1$y != p3$y){
          # Get the coordinates of the 3 points
          
          # Check if there is anything inside the triangle formed by the 3 points
          print(p1)
          print(p2)
          print(p3)
          print("p3")
          convex_hull = insideTriangle(p1,p2,p3,convex_hull)
          if(dim(convex_hull)[1] == 0 ){
            return(convex_hull[!duplicated(convex_hull), ])
          }
        }
      }
...
dftest <- data.frame(x = runif(15,1,15), y = runif(15,1,15))
